...
Work-in-progress interactive explorers built by
...
Each explorer is a self-contained HTML page that works standalone or can be embedded
in course management systems (e.g., Canvas LMS) as an iframe. These are experimental
demonstrations and explorations rather than finished tools, shared early both as a
resource for current and future collaborators and to plant a flag in the underlying
ideas. For the more polished, course- and topic-oriented explainers, see the companion
[Topic Visualizers](https://github.com/tpavlic/topic_visualizers) repository.

Each explorer began life as its own standalone repository; those histories are preserved
and merged into this one, so the per-explorer commit history is still visible here (see
...
| [`rsf_explorer/`](rsf_explorer/)       | Test field hypotheses about how distance interacts with other nest properties in the navigational choices of collective-transport groups and individual ants in polydomous colonies.  |
...
Each explorer was originally maintained as a separate Git repository (hosted as a GitLab/GitHub
...
a single trunk whose history fans back into four independent roots — one per explorer — with every
original commit, author, and date preserved. To inspect a single explorer's history:
...
```bash
my_explorer/
  my_explorer.html          # self-contained page
  my_explorer-preview.png   # preview image for OG/Twitter cards (optional, pending)
```

See [`CLAUDE.md`](CLAUDE.md) for the full per-page checklist (head metadata, Open Graph and
Twitter cards, back-link footer, and index/README entries). The conventions mirror the companion
[Topic Visualizers](https://github.com/tpavlic/topic_visualizers) repository.
